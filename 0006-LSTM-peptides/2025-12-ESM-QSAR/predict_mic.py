...
from train_mic import MLP, BiLSTM_QSAR, BiLSTM_SEQ, BiLSTM_SEQ_ESM, encode_seq
...
def get_args():
    ...
    p.add_argument('--model', type=str, default='mlp',
                        choices=['mlp', 'bilstm', 'bilstm_seq', 'bilstm_seq_esm'],
...
def build_bilstm_seq_esm(ckpt: dict, input_dim: int, device: str | torch.device) -> BiLSTM_SEQ_ESM:
    ...
    print("\n" + "="*60)
    print("STEP 3: Building BiLSTM Seq Model (+ESM)")
    print("="*60)
...
def prepare_bilstm_seq_esm_features(esm, qsar, ckpt, pure_bilstm: bool):
    """
    Prepare x_features for BiLSTM_SEQ_ESM:
      - always standardise pooled ESM using ckpt scaler
      - standardise QSAR only if included (pure_bilstm == False)
      - return X with shape:
            pure: (N, esm_dim)
            not pure: (N, esm_dim + qsar_dim)
    """
    # ESM is always required for bilstm_seq_esm
    if "esm_scaler_mean" not in ckpt or "esm_scaler_scale" not in ckpt:
        raise KeyError("Checkpoint missing ESM scaler (esm_scaler_mean/scale). Re-save ckpt with these keys.")
    ...
    # +QSAR case
    if "qsar_scaler_mean" not in ckpt or "qsar_scaler_scale" not in ckpt:
        raise KeyError("Checkpoint missing QSAR scaler (qsar_scaler_mean/scale).")

    qsar = (qsar - ckpt["qsar_scaler_mean"]) / ckpt["qsar_scaler_scale"]
    return np.hstack([esm, qsar])
...
@torch.no_grad()
def predict_bilstm_seq_esm(model: BiLSTM_SEQ_ESM,
    ...
    with tqdm(total=N, desc="Predicting MIC", unit="peptide") as pbar:
        for start in range(0, N, batch_size):
            ...
            seq_t = torch.tensor(encoded_seq[start:end], dtype=torch.long, device=device)
            x_t   = torch.tensor(x_features[start:end], dtype=torch.float32, device=device)
            len_t = lengths_cpu[start:end]

            out = model(seq_t, x_t, len_t).squeeze(-1).detach().cpu().numpy().astype(np.float32)
            preds[start:end] = out
            pbar.update(end - start)
...
def main(): 
    ...
    if args.model == 'bilstm':
        ...
        print("Building BiLSTM Seq + ESM model..." + (" (pure ESM only)" if args.pure_bilstm else " (+QSAR)"))

        # Build x_features (pooled ESM ± QSAR) using checkpoint scalers
        X = prepare_bilstm_seq_esm_features(esm, qsar, ckpt, pure_bilstm=args.pure_bilstm)

        # input_dim must match what model was trained with
        input_dim = int(X.shape[1])

        # Build model
        model = build_bilstm_seq_esm(ckpt, input_dim=input_dim, device=device)

        sequences, lengths, aa_vocab = load_bilstm_seq_features(args.h5_file, ckpt)
        encoded_seq = encode_seq(sequences, aa_vocab)
        log_mic = predict_bilstm_seq_esm(model, encoded_seq, X, lengths, device, args.batch_size)
